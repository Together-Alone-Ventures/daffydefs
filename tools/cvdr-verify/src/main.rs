...
async fn main() -> Result<()> {
    ...
    println!("[2/6] V1: State transition verification...");
    let v1 = v1_transition::verify(&receipt, canister_id);
    println!("  {}", v1.summary());
    println!();

    // Step 3: V2 — Certificate path
    println!("[3/6] V2: Certificate verification path...");
    ...
    // Step 4: V3 — Module hash (live corroboration, where infrastructure exists)
    println!("[4/6] V3: Module hash verification (live)...");
    let v3 = v3_module::verify(&agent, canister_id, &receipt, published_hash).await;
    println!("  {}", v3.summary());
    println!();

    // Step 5: V3-A — archival attested code identity (offline; embedded certs)
    println!("[5/6] V3-A: Attested code identity (archival, offline)...");
    let v3a = v3_module::verify_v3a(&receipt);
    println!("  {}", v3a.summary());
    println!();

    // Step 6: V4 — Tombstone persistence
    println!("[6/6] V4: Tombstone persistence check...");
    ...
    println!(" {:<16} : {}", "V3-A (attested)", v3a.summary());
    println!(" {:<16} : {}", "V4 (tombstone)", v4.summary());
    println!("============================================================");

    // Exit gate: V1 (transition integrity), V2 (certificate path), and V4
    // (tombstone) remain the hard gates. V3-A gates ONLY on a present-but-invalid
    ...
    if v1.passed() && v2.passed() && v4.passed() && v3a.passed() {
        std::process::exit(0);
    } else {
        std::process::exit(1);
    }
