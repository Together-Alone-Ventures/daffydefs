...
impl NnsRootKey {
    ...
    pub fn raw_bytes(&self) -> Result<&[u8], &'static str> {
        ...
        if self.der_bytes[0] != 0x30 || self.der_bytes[1] != 0x81 || self.der_bytes[2] != 0x82 {
        ...
        if self.der_bytes[34] != 0x03 || self.der_bytes[35] != 0x61 || self.der_bytes[36] != 0x00 {
...
// With `local-replica` the cfg'd early return makes the tail unreachable.
...
pub fn active_key() -> &'static NnsRootKey {
    lookup_key(active_key_id()).expect(
        "active_key_id() returned an ID not in the allowlist — this is a bug in zombie-core",
    )
}
...
mod tests {
    ...
    fn raw_bytes_validates_der_header() {
        ...
        assert!(
            bad_key.raw_bytes().is_err(),
            "wrong DER header should return Err"
        );
    }
    ...
    fn raw_bytes_rejects_short_input() {
        ...
        assert!(
            short_key.raw_bytes().is_err(),
            "short DER should return Err"
        );
    }
